...
By calling the `start()` method with the configuration parameters, the package will begin dispatching tasks while managing threading or processing based on the provided settings. Once the tasks are completed, the package will return a list containing all the return values from each task.

```python
def start(config: dict) -> list
```
...
```python
["Task 1's returned value", "Task 2's returned value"]
```
